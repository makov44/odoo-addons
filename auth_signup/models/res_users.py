...
class ResUsers(models.Model):
    ...
    def signup(self, values, token=None):
        ...
        if token:
            ...
            user = self.with_context(verify_email=True).create(values)
    ...
    def action_verify_email(self):
        ...
        self.mapped('partner_id').signup_prepare(signup_type="verify", expiration=expiration)

        # send email to users with their signup url
        template = False
        if create_mode:
            template = self.env.ref('auth_signup.verification_email', raise_if_not_found=False)
    ...
    def create(self, values):
        ...
        if user.email and bool(self.env.context.get('verify_email')):
            try:
                user.with_context(create_user=True).action_verify_email()
            except MailDeliveryException:
                user.partner_id.with_context(create_user=True).signup_cancel()
        elif user.email and not self.env.context.get('no_reset_password'):
            try:
